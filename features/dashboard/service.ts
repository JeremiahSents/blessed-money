import db from "@/db";
import { loans, billingCycles, payments, appSettings } from "@/db/schema";
...
async function getDashboardStats() {
    ...
    const principalIssuedRes = await db
        .select({ sum: sql<number>`sum(CAST(${loans.principalAmount} AS NUMERIC))` })
        .from(loans);

    const outstandingRes = await db
        .select({ sum: sql<number>`sum(CAST(${billingCycles.balance} AS NUMERIC))` })
        .from(billingCycles)
...
export async function getDashboardData() {
    const [stats, { recentPayments, recentLoans }, overdueLoansList] = await Promise.all([
        getDashboardStats(),
        getRecentActivity(),
        getOverdueList(),
    ]);
